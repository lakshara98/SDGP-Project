...
gazette_filenames = glob.glob("gazette/*.txt")
gazette_filenames

# reading the text file
document = u""
for gazette_filename in gazette_filenames:
    print("Reading '{0}'...".format(gazette_filename))
    with codecs.open(gazette_filename, "r", 'utf-8', 'ignore') as gazette_file:
        document += gazette_file.read()
    print("File is now {0} characters long".format(len(document)))
...
if __name__ == '__main__':
    ...
    # TFIDF trained corupus enlgihs text sum
    # Load the document you wish to summarize
    title = ''
    document = "gazette/*.txt"
